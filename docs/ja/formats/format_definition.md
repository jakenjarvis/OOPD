...
- **見出し (クラス `####`, インターフェース `####`, セクション `###`, 構造体定義 `####`, 列挙型定義 `####`):**
  ...
  - **`#### {構造体定義の固定名称}` / `#### {列挙型定義の固定名称}` (グループ見出し):**
    - `format_common.md` で定義された、言語ごとの **固定名称の見出し** を使用します。
    - これらは、それぞれ構造体クラス定義、列挙型定義をまとめるためのグループ見出しであり、任意で使用できます。
    - 個別の構造体クラス定義や列挙型定義は、必ずこれらのグループ見出しの下にリスト形式 (`-`) で記述されます。
...
- **プロパティ定義:**
  ...
  - 書式 (英): - \`{英語名}: {型名}\`: {説明}
  - **型名:**
    - **標準型/拡張型:** 必ず英語表記 (`String`, `Number`, `List<String>`, `Color` など) を使用します (`format_common.md` 参照)。
...
- **メソッド定義:**
  ...
  - **デフォルト値 (`= {初期値}`):**
    ...
    - **初期値の表記:**
      - リテラル (`True`, `False`, `Null`, 数値, 文字列) はそのまま記述します。
...
- **列挙型定義 (個別):**
  ...
  - **説明:** 各列挙型定義のコロン `:` の後ろに、任意で説明を記述できます。
...
- **基底クラスとインターフェースの実装:**
  ...
  - **継承:**
    - 書式 (日本語例): `**基底クラス:** {基底クラスの母国語名}`
    - 書式 (英語例): `**Base Class:** {BaseClassName}`
    - **【重要】必ず母国語名のみで参照してください。**
  - **インターフェース実装:**
    - 書式 (日本語例): `**インターフェース:** {インターフェース1の母国語名}, {インターフェース2の母国語名}, ...`
    - 書式 (英語例): `**Interfaces:** {InterfaceName1}, {InterfaceName2}, ...`
